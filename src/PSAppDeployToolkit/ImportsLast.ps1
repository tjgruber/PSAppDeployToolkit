...
try
{
    ...
    if (Get-ADTParentProcesses | & { process { if (($_.Name -eq 'PatchMyPC-ScriptRunner.exe') -and ([System.Diagnostics.FileVersionInfo]::GetVersionInfo($_.ExecutablePath).LegalCopyright -match 'Patch My PC')) { return $_ } } })
    {
        $Module.Manifest.FunctionsToExport = $Module.Manifest.FunctionsToExport | & {
            begin
            {
            ...
            process
            {
                if (!$restrictedFunctions.Contains($_))
                {
                    return $_
                }
            }
