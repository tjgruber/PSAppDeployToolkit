    ...
    process
    {
        ...
        try
        {
            try
            {
                ...
                $blockExecCommand = "& (Import-Module -FullyQualifiedName @{ ModuleName = '$("$($Script:PSScriptRoot)\$($MyInvocation.MyCommand.Module.Name).psd1".Replace("'", "''"))'; Guid = '$($MyInvocation.MyCommand.Module.Guid)'; ModuleVersion = '$($MyInvocation.MyCommand.Module.Version)' } -PassThru) { & `$CommandTable.'Initialize-ADTModule' -ScriptDirectory '$($Script:ADT.Directories.Script.Replace("'", "''"))'; `$Script:ADT.Async.LogFileDirectory = '$($adtSession.GetLogPath().FullName)'; `$Script:ADT.Async.LogFileName = '$($adtSession.LogName)'; `$Script:ADT.Async.LogStyle = '$($adtConfig.Toolkit.LogStyle)'; Write-ADTLogEntry -Message `"Showing blocked app dialog for [`$(`$args[0])].`" -Source '$($MyInvocation.MyCommand.Name)'; `$null = & `$CommandTable.'Show-ADTInstallationPrompt$($adtConfig.UI.DialogStyle)' -Title '$($adtSession.InstallTitle.Replace("'","''"))' -Subtitle '$($adtStrings.BlockExecution.Subtitle.($adtSession.DeploymentType.ToString()).Replace("'", "''"))' -Timeout $($adtConfig.UI.DefaultTimeout) -Message '$($adtStrings.BlockExecution.Message.($adtSession.DeploymentType.ToString()).Replace("'", "''"))' -Icon Warning -ButtonRightText OK } ([System.String]::Join(' ', `$args))"
